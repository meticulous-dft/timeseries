...
class MongoDBTimeSeriesClient:
    ...
    def create_time_series_collection(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Creating time series collection: {mongo_config.collection_name}"
            )
    ...
    def create_indexes(self) -> bool:
        ...
        try:
            ...
            # Compound index on meta fields for efficient querying
            self.collection.create_index(
                [("meta.hostname", 1), ("timestamp", 1)], name="hostname_timestamp_idx"
            )

            self.collection.create_index(
                [("meta.region", 1), ("meta.datacenter", 1), ("timestamp", 1)],
            ...
            self.collection.create_index(
                [("measurement", 1), ("timestamp", 1)], name="measurement_timestamp_idx"
            )
    ...
    def setup_sharding(self) -> bool:
        ...
        try:
            ...
            shard_key = {"meta.hostname": "hashed", "timestamp": 1}
    ...
    def insert_documents(self, documents: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            if len(result.inserted_ids) != len(documents):
                logger.warning(
                    f"Expected {len(documents)} inserts, got {len(result.inserted_ids)}"
                )
    ...
    def get_collection_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # MongoDB field name for number of indexes (n + indexes)
            index_count_field = "n" + "indexes"
            return {
                "document_count": stats.get("count", 0),
                "size_bytes": stats.get("size", 0),
                "storage_size_bytes": stats.get("storageSize", 0),
                "avg_document_size": stats.get("avgObjSize", 0),
                "indexes": stats.get(index_count_field, 0),
